...
void Wold_Air()
{
   ...
   Int_t i;
    ...
    //   //--- define some media
    TGeoMedium *Air = new TGeoMedium("Air",1, matNitrogen);

   //--define detecter volume
   TGeoVolume *detec = geom->MakeBox("Detector", Air, length_layer, length_layer,detector_width/2);
   ...
   geom->Export("mygeometry_dd4ENV_Air_world.gdml");
   geom->Export("mygeometry_dd4ENV_Air_world.xml");
   detec->Draw();
   //myhex->Draw();
